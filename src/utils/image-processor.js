 ...
 */
function validateContour(approx, imgWidth, imgHeight, minAreaRatio = 0.25, maxAngleRange = 50) {
  if (approx.rows !== 4) return false;

  const imgArea = imgWidth * imgHeight;
  const area = cv.contourArea(approx);
  if (area < imgArea * minAreaRatio) return false;
  ...
  return angleRange <= maxAngleRange;
}
 ...
 */
export function processImage(cv, sourceCanvas) {
  try {
    ...
    console.log('5️⃣ Applied morphological closing: 9x9 rectangular kernel');

    let edges = new cv.Mat();
    cv.Canny(dilated, edges, 0, 84);
    console.log('6️⃣ Detected edges: Canny(0, 84)');
    ...
    for (let i = 0; i < contourAreas.length; i++) {
      ...
      if (validateContour(approx, resized.cols, resized.rows)) {
        bestApprox = approx;
